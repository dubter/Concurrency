...
// Одни и те же вещи всегда нужно оформлять одинаково:
...
class TicketLock {
 // сначала объявляйте публичные методы, именно они образуют интерфейс вашего класса
 ...
 // менять состояние класса можно только через публичные методы
 private:
...
class OnePassBarrier {
 public: // cначала - конструкторы и публичные методы
  ...
  // если класс не предполагается копировать или перемещать, то явно запретите эти операции

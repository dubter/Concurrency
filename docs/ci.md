...
Пожалуйста, не отправляйте в CI решения, которые не проходят тесты (`clippy test`) локально: там будут запущены те же самые тесты той же самой командой.
...
Токен понадобится утилите `clippy` для создания merge request-ов.

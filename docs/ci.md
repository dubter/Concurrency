...
В CI запускаются те же самые тесты и тем же самым способом: через `clippy test`.
...
Зарегистрируйтесь в [приложении](https://en.wikipedia.org/wiki/HTTP_404).
...
Для каждой задачи в этом репозитории будет создаваться отдельная ветка (например, `all-tasks/1-kv`).

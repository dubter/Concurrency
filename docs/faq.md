...
Запустите исполняемый файл с тестами с флагами `--disable-forks` и `--disable-time-limits`.
...
Да!
Установите свою стартовую директорию в [docker/config/bashrc](/docker/config/bashrc).
...
Для пересборки проекта выполните `Tools` > `CMake` > `Reset Cache and Reload Project`, затем запустите сборку нужной цели.

...
Может помочь перезапуск `clangd`. Для этого удалите директорию `.cache` в корне проекта, зайдите в левом меню в `Extensions` и выключите/включите расширение `clangd`. Позвольте ему закончить индексацию, её процесс можно наблюдать в нижнем меню.

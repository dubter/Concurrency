...
Дуги:
- Когда поток `T` вызывает метод `Lock` на мьютексе `M`, то в графе появляется направленная дуга из `T` в `M` (поток `T` _ждет_ мьютекса `M`).
- Когда вызов `Lock` завершается и поток `Т` захватывает мьютекс `M`, то мы стираем эту дугу и проводим противоположную, от `M` к `T` (мьютексом `M` _владеет_ поток `T`).

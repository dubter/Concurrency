...
Обратите внимание: `futex` ограничен 32-битными словами.
...
`Wake` оборачивает вызов `futex` в цикл с перепроверкой условия:

```cpp
Wake(atomic<uint32_t>& atom, uint32_t old) {
  while (atom.load() == old) {
...
1) Сначала (обязательно _до_ записи в атомик, которая предшествует пробуждению) с помощью `PrepareWait` фиксируется адрес системной очереди ожидания (фактически – адрес ячейки памяти) для атомика:

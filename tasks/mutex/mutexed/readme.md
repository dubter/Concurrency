...
В результате при работе со счетчиком из разных потоков чтения и записи переменной `value_` не будут упорядочены синхронизацией.

Такая ситуация имеет специальное название – [_data race_](https://eel.is/c++draft/intro.races#def:data_race). 

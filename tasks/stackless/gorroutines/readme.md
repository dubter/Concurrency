...
В этом задании асинхронные задачи будут императивно синхронизироваться друг с другом с помощью привычных примитивов `Mutex` и `WaitGroup`, так что мы (в шутку!) назовем их _гор-рутинами_ (_gor-routines_) в честь одного из авторов корутин в С++ – [Гора Нишанова](https://www.youtube.com/watch?v=_fu0gx-xseY).

Не путайте их с [настоящими горутинами](https://gobyexample.com/goroutines) (goroutines) из языка Go!

### Функциональность

В то же время фреймворк задач позволяет трансформировать и комбинировать асинхронные задачи в виде экземпляров `Task<T>` в функциональном стиле, этому будет посвящена отдельная задача.
...
void GorRoutinesExample() {
  ...
  auto main = [&]() -> tasks::Task<> {
    ...
    auto contender = [&]() -> tasks::Task<> {
      co_await tasks::TeleportTo(scheduler);
      // <-- Теперь корутина исполняется в пуле независимо от других
    ...
    // В этой задаче для простоты считаем, что лок
    co_await wg.Wait();
  };
...
Как и файберы, корутины могут синхронизироваться через известные примитивы синхронизации, принципиальных отличий от файберов нет. В этой задаче мы реализуем [`Mutex`](exe/tasks/sync/mutex.hpp) и [`WaitGroup`](exe/tasks/sync/wait_group.hpp).

### Реализация

В реализации примитивов синхронизации не должно быть динамических аллокаций памяти. Очереди ожидания в `Mutex` и `WaitGroup` сделайте интрузивными, узлами снова будут awaiter-ы.

В реализации `Lock` мьютекса или `Wait` у `WaitGroup` вы больше не сможете написать цикл `while` с засыпанием внутри, `co_await` предусматривает только одну остановку корутины (точнее, не более одной).
...
Обратите внимание: корутины, в отличие от файберов, не привязаны к планировщику, что создает трудности при их пробуждении. 
...
Попробуйте теперь материализовать состояние `2` в явный интрузивный список awaiter-ов ждущих корутин.

#### `WaitGroup`

в `await_ready` для awaiter-а `WaitGroup`  оптимистично проверяйте счетчик работы.

Выделите сложный случай в отдельный класс `OneShotEvent`.
...
Если подставить в реализацию быстрый планировщик с локальными лок-фри очередями в исполнении корутин почти не будет взаимного исключения!

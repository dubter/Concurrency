...
`Strand` не делает никаких предположений об устройстве декорируемого экзекутора. Единственная гарантия, на которую `Strand` может полагаться, – каждая запланированная
...
Чем больше число ядер / потоков, исполняющих критические секции, тем больше накладные расходы в самом мьютексе и тем меньше пользы от кэшей, так как разделяемые данные, к которым обращаются критические секции, придется постоянно двигать между кэшами ядер.

##### Strand

Гораздо разумнее двигать не данные к критическим секциям на разные ядра, а критические секции перенести на одно ядро.
...
- И наоборот, `Strand` не должен искусственно задерживать исполнение задач.

#### Lock-Free

Простая реализация `Strand` будет использовать взаимное исключение для доступа к очереди задач `Strand`-а.
...
Избавьтесь от динамических аллокаций памяти в экзекуторах, сделав задачи интрузивными. За аллокацию памяти для задачи должен отвечать пользователь экзекутора.

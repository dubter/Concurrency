...
Эта декомпозиция делает пул потоков универсальным: он в равной степени подходит для всех перечисленных выше выразительных средств.

## Абстракция

В этой задаче мы абстрагируем среду исполнения: файберы, фьючи и корутины будут запускать свои задачи не в конкретном пуле потоков, а в абстрактном _экзекуторе_.
...
Использовать мьютексы нельзя: если задача в пуле захватила мьютекс на продолжительное время, то другие задачи на других потоках пула заблокируются, тем самым заблокировав исполнение другим задачам, которые могут выполняться и которым мьютекс не нужен.

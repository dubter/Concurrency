...
wheels::Result<T> Future<T>::GetResult() && {
  return ReleaseState()->GetResult();  // Not implemented
}
...
template <typename T>
Future<T> Future<T>::Via(executors::IExecutorPtr) && {
...
template <typename T>
template <typename U>
Future<U> Future<T>::Transform(Transformer<U>) && {
  return Future<U>::Invalid();  // Not implemented
}

template <typename T>
template <typename U>
Future<U> Future<T>::Transform(AsyncTransformer<U>) && {
  return Future<U>::Invalid();  // Not implemented
}

...
В первом и очень грубом приближении `Future` / `Promise` реализуют канал для возврата _результата_ (`Result`) из асинхронной операции.

- `Promise<T>` – конец канала для записи, через него поток-_продьюсер_ отправляет результат,
- `Future<T>` – конец канала для чтения, через него поток-_консьюмер_ поглащает асинхронный результат.
- `Result<T>` – контейнер, содержащий значение типа `T` либо ошибку

```cpp
executors::ThreadPool pool{4};

// Разворачиваем канал, создаем "контракт" на передачу значения
auto [f, p] = futures::MakeContract<int>();

// Запускаем асинхронную операцию
executors::Execute(pool, [p = std::move(p)]() {
  // Отправляем результат
  // В общем случае – Set(Result<T>)
...
Конструировать экземпляры `Result` стоит с помощью статических конструкторов или свободных функций из пространства имен [`make_result`](https://gitlab.com/Lipovsky/wheels/-/blob/master/wheels/result/make.hpp).
...
Такой взгляд на фьючи открывает дорогу к функциональной композиции асинхронных операций: теперь мы
можем оперировать асинхронными вычислениями как значениями с помощью специальных функций-_комбинаторов_.
...
// Продолжение ожидает значения типа T, а не фьючу
...
Продолжение получает единственным аргументом значение типа `T` и возвращает значение типа, который мы будем обозначать через `U`.
...
Ошибка, возникшая на одном из шагов асинхронного конвейера, будет "пролетать" сквозь последующие `Then` без вызова продолжений:
...
Вариации `Then` связывают вычисления в цепочки: будущее значение, представленное в виде `Future<T>`, связывается с продолжением, которое ожидает на вход "распакованное" значение типа `T`.

В обоих случаях у нас есть 
- _контейнер_ для значения и 
- правило _связывания_ вычислений.

Эти точки кастомизации образуют понятие _функтора_ или _монады_, см. [Functors, Applicatives, And Monads In Pictures](https://adit.io/posts/2013-04-17-functors,_applicatives,_and_monads_in_pictures.html).
...
конкурирующие графы обработки пользовательских запросов.

Декларативный стиль скрывает от пользователя всю внутреннюю машинерию, все детали синхронизации, переводя фокус с _control flow_ на _data flow_.
...
Фьючи позволяют декларативно описывать последовательные цепочки асинхронных операций с помощью комбинаторов `Then` и `Recover`.
...
Файберы и фьючи – не исключающие друг друга альтернативы, а равноправные инструменты, которые следует комбинировать друг с другом для максимальной выразительности кода.
...
Коллбэк асинхронно поглощает результат фьючи: к фьюче можно подвесить только один коллбэк, после чего обращаться к ней больше нельзя.

### Примеры коллбэков

Как правило, пользователи фьюч не вызывают `Subscribe` напрямую, а используют более высокоуровневые операции, которые в свою очередь подвешивают к фьючам служебные коллбэки:
...
Обратите внимание: у `Future` нет блокирующих методов!

Фьючи – _неблокирующие_, они не ждут, а реагируют на события: при получении результата фьюча запускает коллбэек, т.е. следующий шаг вычисления / следующую асинхронную операцию / возобновляет ждущий ее файбер или корутину.

Соответственно, в реализации `Future` / `Promise` не должно быть блокирующих поток примитивов синхронизации, не должно быть взаимного исключения. 

...
Правда при этом поток управления (_control flow_) выворачивается наизнанку: теперь он следует не вашей логике (как в случае с потоками), а подчиняется циклу внутри `io_context`-а, фактически – `epoll`-у.

Ваш код разрывается в точках выполнения I/O на фрагменты-коллбэки, вы не можете писать циклы, использовать исключения, снимать стек-трейсы. Такую цену вы платите за масштабируемость вашего сетевого кода.

### Файберы

Файберы разрешают дилемму между простотой и масштабируемостью, они сочетают преимущества _обоих_ подходов.
...
Если вы проигнорируете проверку `Result`-а, который получили из вызова метода или функции, то компилятор сгенерирует предупреждение (а с флагом `-Werror` – ошибку), см. [`[[nodiscard]]`](https://en.cppreference.com/w/cpp/language/attributes/nodiscard)
...
Можно строить `Result`-ы с помощью статических конструкторов `Ok` и `Fail`, но гораздо удобнее пользоваться свободными функциями из пространства имен `make_result`, они возьмут на себя вывод шаблонного типа:
...
if (result.HasError()) {
  ...
  // не обязательно тот же, что и `ReadSome`

...
Методы `Mutex::Lock`, `CondVar::Wait` и `WaitGroup::Wait` должны блокировать только вызывающий их файбер, но не блокировать поток планировщика, в котором этот файбер исполняется.
...
Примеры:
- `ParkIfEqual` на фьютексе: запланировать возобновление – значит положить себя в очередь ожидания, из которой файбер потом достанут и запустят в вызовах `WakeOne` / `WakeAll`.
...
В вашей реализации фьютекса (и в примитивах на его основе) не должно быть динамических аллокаций памяти.
...
`WaitGroup` тривиально реализуется с помощью мьютекса и кондвара.
...
Использовать внутри `Fiber` взаимное исключение нельзя. Ждать другой поток тоже нельзя.

Менять API пула потоков не нужно, файберы по-прежнему полагаются для планирования только метод `Submit`.

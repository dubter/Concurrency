...
В этой задаче мы хотим придумать универсальный `Suspend`, который подойдет для всех перечисленных выше сценариев.
...
Экземпляр `FiberHandle` представляет собой **остановленный** файбер.

Запустить его можно двумя способами:
- `Schedule`: запланировать файбер в планировщик
- `Resume`: возобновить файбер прямо на текущем потоке

Непрозрачный `FiberHandle` позволит 
- Избавиться в реализации примитивов синхронизации от непосредственного доступа к объекту `Fiber`, скрыть его от пользователя
- Избавить `Fiber` от знания про примитивы синхронизации

FiberHandle – TriviallyCopyable, передавайте его по значению.
...
В реализации `Mutex`, `Event` и `WaitGroup` вам потребуются очереди ожидания. Реализуйте их без динамических аллокаций памяти.
...
Подумайте, какой объект лучше всего сделать узлом интрузивного списка.
...
Не делайте `FiberHandle` узлом интрузивного списка. Списки – это подробность реализации примитивов синхронизации,
и эта подробность должна быть локализована в них.
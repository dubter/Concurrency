...
Методы `Mutex::Lock`, `CondVar::Wait` и `WaitGroup::Wait` должны останавливать только вызвавший их файбер, но не должны блокировать поток планировщика, в котором этот файбер исполняется.
...
- `void Wait()` – ждать (остановив файбер), пока значение счетчика не опустится до нуля
...
- Блокирующий `Await` на [`Future`](/tasks/futures/futures): запланировать возобновление – значит подписаться на готовность `Future` с помощью `Subscribe` и запустить себя в коллбэке.
...
_Awaiter_ решает когда именно _остановленный_ файбер должен запуститься.
...
Выразите функцию `Yield` через функцию `Suspend`. 
...
После реализации лок-фри `Mutex` и `WaitGroup` вы увидите, что файберам вовсе не нужен `FutexLike`. _Всегда_ разумно пользоваться 
напрямую `Suspend`-ом.
...
6) Бонусный уровень: реализуйте лок-фри мьютекс c серийным запуском критических секций.
8) Бонусный уровень: реализуйте лок-фри `WaitGroup`.

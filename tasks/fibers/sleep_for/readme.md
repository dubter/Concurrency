...
Попробуйте написать код так, чтобы из методов файбера только `SleepFor` знал про таймеры. Файбер не хочет знать про каждый новый IO object или примитив с синхронизации, с которыми он будет работать.

...
Дайте пользователю (например, файберам) возможность управлять планированием: для этого поддержите в методе `Execute` (+ в `FiberHandle::Schedule`) дополнительный параметр `Hint` – подсказку планировщику.

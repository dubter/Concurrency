...
При этом процедура планирования задачи упирается в общую очередь, т.е. в общий мьютекс, которая и ограничивает масштабируемость: число критических секций в единицу времени для мьютекса с ростом числа ядер не растет, а скорее уменьшается.
...
Если воркер не может найти доступных задач ни локально, ни в глобальной очереди, ни у других воркеров, то он должен заблокироваться.
...
Если при планировании оказывается, что в LIFO-слоте уже есть задача, то вытесняйте ее в хвост локальной очереди.

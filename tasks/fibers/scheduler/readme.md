...
В случае файберов задачи получаются очень короткими, так что накладные расходы на планирование этих задач становятся сопоставимы с временем их работы.

При этом процедура планирования задачи упирается в общую очередь, т.е. в общий мьютекс, которая и ограничивает масштабируемость:

Число критических секций в единицу времени для мьютекса с ростом числа ядер не растет, а скорее уменьшается.
...
Если такие задачи зависимые задачи запускать на одном ядре и последовательно, то можно получить выигрыш за счет того, что получатель 
...
У каждого потока-воркера есть собственная ограниченная очередь задач (будем называть эту очередь _локальной_), воркер работает в первую очередь с ней: забирает из нее и планирует в нее задачи. 
...
2) Парковку воркеров, так как больше нельзя атомарно проверить пустоту общей очереди и запарковать поток.

### Балансировка нагрузки

Воркеры разделяют общую очередь задач неограниченной емкости (будем называть ее _глобальной_):
- в нее они выгружают излишки своих задач, 
- из нее они забирают новые задачи, когда локальная очередь опустошается.

В случае, когда и локальная, и глобальная очереди пусты, воркеры воруют задачи прямо из локальных очередей других воркеров ([Work stealing](https://en.wikipedia.org/wiki/Work_stealing)).
...
- Если задача планируется из воркера пула, то она добавляется в хвост локальной очереди либо в LIFO-слот, если пользователь явно попросил о последнем с помощью специальной подсказкич.
...
Если воркер не может найти доступных задач должен парковаться, уходить с ядра. Про парковку см. пункт "Координация".
...
Файбер сам является интрузивной задачей!

Унаследуйте `Fiber` от `TaskBase` и в методе `Run` делайте шаг корутиной.
...
Методы `TryPush` и `TryPop` вызывает только владеющий им воркер.
Метод `Grab` может вызывать
- сам воркер когда перекладывает задачи в глобальную очередь,
- другие воркеры когда воруют задачи из чужих локальных очередей.
...
половина задач выгружается в глобальную очередь с помощью метода `OffloadTasksToGlobalQueue`.
...
Если локальная очередь воркера пустеет, то воркер пытается забрать пачку задач из глобальной очереди с помощью `GrabTasksFromGlobalQueue`.
...
Поддержите LIFO-слот при планировании: задача, запланированная прямо из пула потоков, попадает в этот слот и запускается уже на следующей итерации цикла планирования в воркере.

Если при планировании оказывается, что в слоте уже есть задача, то вытесняйте ее в хвост локальной очереди.

Ограничьте число последовательных итераций планирования через LIFO-слот для обеспечения честности.

Дайте пользователю (например, файберам) возможность управлять планированием: для этого поддержите в методе `Execute` дополнительный параметр `Hint` – подсказку планировщику.
...
Изучите [механизм парковки потоков (машин)](https://github.com/golang/go/blob/d2552037426fe5a190c74172562d897d921fe311/src/runtime/proc.go#L31) в планировщике `Go`.
...
Используйте аналогичную двухфазную парковку в своей реализации, реализуйте ее через класс [`Coordinator`](exe/executors/ftp/coordinator.hpp). 
...
- Сколько задач было запущено через LIFO-слот / локальную очередь / взято из глобальной очереди
...
Профилируйте вашу реазализацию на [`workloads`](workloads/main.cpp) чтобы понять на что вы расходуете время при планировании.

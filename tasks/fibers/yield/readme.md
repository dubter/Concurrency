...
- Не аллоцируйте в нем стеки, получайте стек в виде `wheels::MutableMemView`: управлением ресурсов должен заниматься отдельный компонент
- Не решайте в нем задачу поиска текущей корутины: файберам потребуется свой собственный механизм поиска, а для генераторов удобнее реализовать `SendContext`
- Не выполняйте в нем type erasure, а передавайте указатель на реализацию интерфейса `IRunnable` с методом запуска процедуры пользователя: в файберах мы сможем избавиться от дополнительной аллокации для исполняемой процедуры

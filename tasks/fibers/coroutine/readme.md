...
Чтобы лучше понять предложенную декомпозицию, подумайте над соотношением сущностей _задача_ (`Task`), _корутина_ (`Coroutine`) и _файбер_ (`Fiber`).
...
Будем считать, что все пользователи пула, которые запускают в нем лямбды, пользуются не методом 

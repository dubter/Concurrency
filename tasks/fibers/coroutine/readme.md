...
`CoroutineImpl` не управляет стеками, он получает только _view_ на них (в виде экземпляра `MutableMemView`), за управление памятью отвечает код уровнем выше (например, рантайм файберов).
...
Поэтому пара советов на будущее:

Не используйте напрямую метод `Submit` у пула потоков, вместо это используйте
свободную функцию [`Submit`](exe/tp/submit.hpp). В тестах метод `Submit` не используется.

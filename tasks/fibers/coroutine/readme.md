...
Экземпляр класса `Coroutine` представляет вызов сопрограммы,
...
Корутины – базовый инструмент для выражения конкурентности и работы с асинхронностью в современных языках программирования.
...
Планировщик для файберов вами уже реализован – это пул потоков, `ThreadPool`.

## Файберы

```cpp
void FibersExample() {
  // Наш фреймворк для concurrency
  using namespace exe;
...
Пул потоков ничего не знает про природу задач, которые он исполняет, и может быть полезен для задач распараллелилвания вычислений.
...
Мы перечислили разные применения сопрограмм: файберы, генераторы и т.п.:

В нашем фреймворке они будут основаны на одной и той же реализации – [`CoroutineImpl`](exe/coroutine/impl.hpp).

`CoroutineImpl` не управляет стеками, он получает только _view_ на них (в виде экземпляра `MutableMemView`), за управление памятью отвечает код уровнем выше (например, файберы)

`CoroutineImpl` имеет небезопасный / неудобный `Suspend`: пользователь должен сам найти объект корутины и позвать метод прямо на нем. 
...
Текущее состояние исполнения на процессоре будет зафиксировано в `current`, после чего на процессоре активируется контекст `target`.
...
Для файберов реализуйте [`StackAllocator`](exe/fibers/core/stack.cpp), который переиспользует стеки завершившихся файберов.

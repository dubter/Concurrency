...
Будем считать, что все пользователи пула потоков планируют в нем задачи
не напрямую, а через свободную функцию `tp::Submit`:
...
Благодаря `tp::Submit` в файберах можно избавиться от аллокаций во время 
планирования задач.
